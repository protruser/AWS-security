...
Node.js 22.12 이상을 설치한 뒤 `frontend/` 폴더에서 실행합니다.
...
```text
dashboard/
├─ frontend/    React 화면 (package.json, vite.config.ts, src/ …). npm 명령은 여기서 실행
...
- `frontend/src/data/architecture.ts`: 리소스 정보·상태·Grid 좌표
- `frontend/src/data/architectureLayout.ts`: 서브넷과 외부 Anchor·연결 경로
- `frontend/src/data/mock.ts`: 원본 이벤트·탐지/조치 이력·시나리오·챗봇 예시 (DB 연결 실패 시 이 데이터를 그대로 사용)
- `frontend/src/data/types.ts`, `frontend/src/index.css`: 공통 타입·스타일
...
```sh
cd frontend && node scripts/browser-check.mjs
```
